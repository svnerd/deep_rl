...
class ExperienceMemory:
    ...
    def sample(self, dim_tensor_maker):
        ...
        b_states = np.stack([s.state for s in samples])
        b_actions = np.stack([s.action for s in samples])
        b_rewards = np.vstack([s.reward for s in samples])
        b_next_states = np.stack([s.next_state for s in samples])
        b_dones = np.vstack([s.done for s in samples]).astype(np.uint8)
        #print(b_states.shape, b_actions.shape, b_rewards.shape, b_next_states.shape, b_dones.shape)
